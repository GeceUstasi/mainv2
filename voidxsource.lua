    ...
    function window:CreateTab(tabName, tabIcon)
        ...
        function tab:CreateDropdown(options)
            ...
            local function createOptions()
                ...
                -- Update layout
                dropdownListLayout:ApplyLayout()
            end
            ...
            local function setupDropdownToggle()
                if dropdownConnection then
                    dropdownConnection:Disconnect()
                end
                
                dropdownConnection = dropdownButton.MouseButton1Click:Connect(function()
                    isOpen = not isOpen
            ...
            local element = {
                ...
                AddOption = function(option)
                    ...
                    element.UpdateOptions(dropdownList)
                end,
        ...
        function tab:CreateSearch(options)
            ...
            if autoRefresh and getItems then
                ...
                spawn(function()
                    while refreshIndicator and refreshIndicator.Parent do
                        if refreshIndicator and refreshIndicator.Parent then
                            refreshIndicator.BackgroundTransparency = 0.5
                        end
                        wait(0.5)
                        if refreshIndicator and refreshIndicator.Parent then
                            refreshIndicator.BackgroundTransparency = 0
                        end
                        wait(0.5)
                    end
                end)
            ...
            local function updateResults(query)
                ...
                for i = 1, maxResults do
                    ...
                    resultButton.MouseLeave:Connect(function()
                        resultButton.BackgroundTransparency = 0.9
                        resultButton.TextColor3 = window.Theme.TextDim
                    end)
            ...
            local function refreshList()
                if getItems then
                    local success, newList = pcall(getItems)
                    if success and newList then
                        currentList = newList
                        itemCount.Text = "(" .. #currentList .. ")"
                        ...
                        if refreshIndicator then
                            ...
                            task.wait(0.1)
                            if refreshIndicator and refreshIndicator.Parent then
                                refreshIndicator.BackgroundColor3 = window.Theme.Accent
                            end
                        end
                    end
                end
